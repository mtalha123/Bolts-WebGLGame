...
void main()
{
    ...
    for(int i = 0; i < 6-1; i++){
        ...
        lineWidthUV_t = transformToAspectRatioMeasurement(aspectRatio, lineWidth / iResolution.y, angle);
        
        t_fluctuation = fluctuation + (fluctuation *  cos(abs(angle)) * (aspectRatio - 1.0));
        ...
        float pureNoiseVal = texture2D(noise, (vec2(t_xClamped, iGlobalTime / 1024.0))).r;
        pureNoiseVal = map(0.0, 1.0, -1.0, 1.0, pureNoiseVal);
    	t_yNoiseVal = pureNoiseVal * ( (t_fluctuation * lightningCos(t_xClamped, t_lengthOfLightning)) / iResolution.x);
    ...
    float multiplier = invertedDistance * (1.0 / 300.0);// * (glowFactor / iResolution.x);
    ...
    //-------------------------------
   float alpha = 1.0 - smoothstep(0.001, 0.015, distanceToPoint);
